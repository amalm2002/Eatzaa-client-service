...
const OrderHistory: React.FC<OrderHistoryProps> = ({ tealColor = '#2C938C' }) => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        setLoading(true);
        const response = await userApi.getUserOrders(dispatch, userId, currentPage, ordersPerPage);
        ...
        const enrichedOrders: Order[] = responseData.data.orders.map((order: any) => {
  ...
  }, [userId, currentPage, ordersPerPage, dispatch]);
