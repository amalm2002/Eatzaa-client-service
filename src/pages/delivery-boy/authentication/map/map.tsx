...
import { MapProps } from '../../../../interfaces/delivery-boy/map/map.types';
...
export default function RegisterMap({ latitude, longitude, onLocationChange }: MapProps) {
    ...
    const [deliveryBoyPosition, setDeliveryBoyPosition] = useLocalStorage('DELIVERY_BOY_MARKER', {
        latitude: latitude,
        longitude: longitude
    })
    ...
    useEffect(() => {
        if (!mapRef.current) {
            ...
            mapRef.current.on('click', (e: leafLet.LeafletMouseEvent) => {
                ...
                setDeliveryBoyPosition({ latitude: lat, longitude: lng })
        ...
        if (mapRef.current) {
            if (restaurantMarkerRef.current) {
                ...
                restaurantMarkerRef.current.on('dragend', function (event) {
                    ...
                    setDeliveryBoyPosition({ latitude: position.lat, longitude: position.lng });
            ...
            setDeliveryBoyPosition({ latitude, longitude });
