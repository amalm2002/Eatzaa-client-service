...
import { VehiclePageProps } from "../../../../interfaces/delivery-boy/vehicle/vehicle.types";
...
const VehiclePage: React.FC<VehiclePageProps> = ({ vehicle, setVehicle, handleNavigation }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (vehicle) {
            setError('');
            try {
                const data = await deliveryBoyApi.submitVehicleSelection(dispatch, deliveryBoyId, vehicle);
                toast.success(data.message);
                handleNavigation('zone');
            } catch (error: any) {
                toast.error(error.message || 'Error submitting vehicle selection');
            }
        } else {
