...
import { CartItemProps } from '../../../interfaces/user/cart/cart-item.types';
...
const CartItem: React.FC<CartItemProps> = ({ item, onUpdateQuantity, onRemove }) => {
  ...
  const handleIncrease = () => {
    ...
    if (newQty <= item.maxAvailableQty) {
      setLocalQty(newQty);
      ...
      <div className="p-4">
        <div className="flex gap-3">
          ...
          <div className="relative flex-shrink-0">
            ...
            />
            <span
              className={`absolute -top-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${item.category.toLowerCase() === 'veg' ? 'bg-green-500' : 'bg-red-500'
                }`}
            />
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex justify-between items-start mb-1">
              <div className="flex-1 min-w-0 pr-2">
                ...
                <p className="text-xs sm:text-sm text-gray-600 mb-1">{item.restaurant}</p>
              </div>
              <button
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex flex-col">
                ...
                {localQty > 1 && (
                  <span className="text-sm text-gray-600">Total: ₹{item.price * localQty}</span>
                )}
              </div>
